...
def lancement():
    ...
    taille_sac_plastique = (80, 80)  # Spécifiez les dimensions souhaitées
    ...
    while not jeu_termine:
        ...
        if pygame.time.get_ticks() - temps_debut >= random.randint(500, 1500):
            hauteur = random.randint(100, 500)  # plage de hauteur de sorti des poissons
            angle = random.uniform(-math.pi / 4, math.pi / 4)
            poissons.append(Poisson(0, hauteur, angle))
            temps_debut = pygame.time.get_ticks()

        # Probabilité d'ajouter un nouveau sac plastique
        if random.random() < 0.006:  # une probabilité de 0.8 d'avoir un sac plastique à chaque itération
            hauteur = random.randint(100, 500)  # plage de hauteur de sortie des sacs plastiques
            angle = random.uniform(-math.pi / 4, math.pi / 4)
